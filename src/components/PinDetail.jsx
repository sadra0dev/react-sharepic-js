import {sanity, urlFor, pinDetailMorePinQuery, pinDetailQuery} from "configs"
import {MdDownloadForOffline, Spinner, MasonryLayout} from "components"
import {useEffect, useState, uuidv4, Link, useParams} from "libs"
...
export const PinDetail = ({user}) => {
  ...
  const fetchPinDetails = () => {
    sanity.fetch(pinDetailQuery(pinId)).then((data) => {
      setPinDetail(data)
      if (data) {
        sanity.fetch(pinDetailMorePinQuery(data)).then((res) => {
          setPins(res)
        })
      }
    })
